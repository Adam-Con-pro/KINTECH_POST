/**
  ...
  V1. Org Version  44145

  V2. ADAM @ KT 11-6-24
    -Added Username and Date
    -Removed Probe Rotate on 9832 and rotate off 9833 
    -Added Option to force "Y" Axis Home at the end of program

  V3. ADAM @ KT 11-6-24
    -Added manual nc codes to post prior to a section. 
    All Manual NC codes will be output in the order they are in fusion.
    executeManualNC(); Nc must be in section open, section end and on close.
    -changed program title to be the title of the setup in fusion instead of comment

  V4. ADAM @ KT 11-7-24
    -Added Switch to use either Comments or Setup description as program name
      
*/

// ATTENTION: parameter F86 bit 6 must be on for G43.4

description = "Mazak - MILL - KT";
vendor = "Mazak";
...
properties = {
  ...
  Force_Y_Home: {
    title      : "Force Y Axis Home",
    description: "Outputs optional Y home postion at the end of the program",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  useComment: {
    title      : "Use Comment For Program Name",
    description: "Outputs a Comment instead of part description at start of the program",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
var maximumLineLength = 80; // the maximum number of charaters allowed in a line // Added ADC 11-6-24 part of V3. edit
...
function onOpen() {
  ...
  // V4. Added Switch for comments as Program name here Adc 11-6-24
  if(getProperty("useComment")){
  ...
  // V3. End Added description to program here
...
// V2. ADDED USER NAME AND DATE ADC 11-6-24
writeln("");
if (hasGlobalParameter("username")) {
  var User = getGlobalParameter("username");
  writeComment(localize("Username:") + " " + User.toLocaleString());
  }

  var d = new Date();{ // output current date and time
    writeComment(localize("Posted On") + ": " + d.toLocaleDateString() + " " +
    d.toLocaleTimeString());
}

  // End username and date
  ...
  writeln("");
  if (tcp.isSupportedByControl) {
    for (var i = 0; i < getNumberOfSections(); ++i) {
      var section = getSection(i);
      if (isTCPSupportedByOperation(section)) {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || state.tcpIsActive) {
    ...
    // V3. ADD end of section items here ADC 11-6-24
    executeManualNC(); // V3. ADDED ALL MANUAL CODES OUTPUT HERE IN ORDER OF OPERATIONS IN FUSION. See Notes ADC 11-6-24
  ...
  if (isProbeOperation()) {
    ...
    //writeBlock(settings.probing.macroCall, "P" + 9832); // spin the probe on // V2. Removed, caused alarm at Machine ADC 11-5-24
...
// V2. Added Manual NC funtions here ADC 11-5-24
/**
...
function onManualNC(command, value) {
  ...
  */
  ...
  for (var i = manualNC.length -1; i >= 0; --i) { // <<< ADD THIS LINE
    if (!command || (command == manualNC[i].command)) {
      manualNC.splice(i, 1);
    }
  }
...
// end Manual NC addition ADC 11-5-24
...
function onCommand(command) {
  switch (command) {
  case COMMAND_STOP:  // Removed adam
    writeBlock(mFormat.format(0));
    forceSpindleSpeed = true;
    forceCoolant = true;
    return;
...
function onSectionEnd() {
  ...
  if (isProbeOperation()) {
    //writeBlock(settings.probing.macroCall, "P" + 9833); // spin the probe off // V2. Removed, caused alarm at Machine ADC 11-5-24
...
function onClose() {
  ...
  if (typeof inspectionProcessSectionEnd == "function") {
   ...
   // V2. Added Force Y home Adc 11-5-24
   {
    if (getProperty("Force_Y_Home")) 
      writeRetract(Y);
      yOutput.reset();
    } 
    // End force home
...
function disableLengthCompensation(force) {
  if (state.lengthCompensationActive || force) {
    ...
    writeBlock(toolLengthCompOutput.format(49));// possible remove at end of section ADC 11-5-24
  }
}
// <<<<< INCLUDED FROM include_files/disableLengthCompensation_fanuc.cpi
